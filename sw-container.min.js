...
var kv_default2 = (...args) => query(...args);
...
var getOriginPrivateDirectory_default2 = getOriginPrivateDirectory;
...
globalThis.loadLocalServiceWorker = async () => {
  root ??= await kv_default2("get", "root").then(
    (dir) => dir?.type === "jsdelivr" ? getOriginPrivateDirectory_default2(jsdelivr_default, dir.root) : dir
  );
...
globalThis.fetch = async function fetch2(...args) {
  ...
  if (url.href.startsWith(origin)) {
    ...
    root ??= await kv_default2("get", "root");
    if (root?.type === "jsdelivr") {
      root = await getOriginPrivateDirectory_default2(jsdelivr_default, root.root);
    }
    if (root) {
    ...
    const zipBlob = await kv_default2("get", "zip-file");
    if (zipBlob) {
